...
class Page1:
    ...
    def click_btnYes(self):
        ...
        self.count = change_date.count('')
        for i in range(self.count):
            change_date.remove('')

        self.date = []
        for i in range(len(change_date)):
            self.date.append([change_date[i]])

        for i in range(len(self.date)):
            self.date[i] = self.date[i][0].split('/')

        self.date = sorted(self.date, key=lambda i : (int(i[0]), int(i[1]), int(i[2])))

        change_date = []
        for i in range(len(self.date)):
            change_date.append( '/'.join(self.date[i]))
