...
class Page1:
    ...
    def __init__(self, master=None):
        ...
        # 不知道為什麼加照片一直失敗
        # self.imageSearch = Image.open('search.png')
        # self.imageSearch = self.imageSearch.resize((50, 50))
        # self.btnSearch = tk.Button(self.frame_context1, image=self.imageSearch, text = "搜尋", compound = "left", command=self.click_btnSearch)

        
        self.lblTitle_A.place(x=0, y=50)
        self.btnCreate_New.place(x=780, y=50)
        self.lblSearch.place(x=480, y=55)
        self.btnSearch.place(x=700, y=53)
        self.inputKey.place(x=570, y=55)
        ...
        for i in range(len(meeting_names)):
            ...
            font = tkfont.Font(size=30-length, family="源泉圓體 B")  # 讓字隨字數變大而縮小
            self.btn_names = tk.Button(self.frame_context1, text= meeting_names[i], image = self.pixel, relief='solid',
                                       font = font, height=120, width=252, compound="center", wraplength=200, justify="left",
    ...
    def create_window(self):
        ...
        self.btnYes = tk.Button(self.window, text="確定", height=1, width=5, bg=self._from_rgb((255, 217, 102)), font=f2,
...
class PageSearch:
    ...
    def __init__(self, master=None):
        ...
        for i in range(len(fit_meetings)):
            ...
            self.btn_names = tk.Button(self.pageS, text= fit_meetings[i], image = self.pixel, relief='solid',
                                       font = font, height=120, width=252, compound="center", wraplength=200, justify="left",
                                       bg='white', command=lambda a=i: self.click_btn_meetings(a))
        ...
        self.lblText = tk.Label(self.pageS, text = "符合\""+keywords.get()+"\"的會議共有"+str(counts)+"個：", font=f2, bg=color_2)
